...
cls_labels_dict = np.load('data/VOCdevkit/voc12/cls_labels.npy', allow_pickle=True).item()
...
def load_img_name_list(dataset_path):
    ...
    img_name_list = np.array([int(i) for i in img_name_list])
    # img_name_list = np.loadtxt(dataset_path,delimiter='_', dtype=np.int32)
...
class VOC12ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = np.asarray(imageio.imread(get_img_path(name_str, self.voc12_root))) # (h, w, 3)

        if self.resize_long:
            img = random_resize_long(img, self.resize_long[0], self.resize_long[1]) # random number(320-640) k -> (h*k/w, k, 3)
        if self.rescale:
        ...
        if self.crop_size:
            if self.crop_method == "random":
                img = random_crop(img, self.crop_size, 0) # (512, 512, 3)
            else:
...
class VOC12ClassificationDatasetMSF(VOC12ClassificationDataset):
    ...
    def __getitem__(self, idx):
        ...
        out = {"name": name_str, "image_data": ms_img_list, "size": (img.shape[0], img.shape[1]),
               "gt_cls": torch.from_numpy(self.label_list[idx])}
        return out
