...
class CAM_Processor():
    ...
    def load_scheduler(self):
        if self.arg.scheduler == 'ReduceLROnPlateau':
            ...
            self.scheduler = optim.lr_scheduler.StepLR(self.optimizer, step_size=1, gamma=0.3)
        else:
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, item in enumerate(self.data_loader['train']) :
            self.global_step += 1
            # breakpoint()
            data = item['img'].cuda(self.output_device) # (batchsize, 3, 512, 512)
            gt_cls = item['label'].cuda(self.output_device) # (batchsize, num_classes) : multi-label
        
            # forward
            output = self.model(data)[0] # (batchsize, num_classes) 
    ...
    def test(self, epoch):
        ...
        self.print_log("\t Mean test loss: {:.4f}".format(np.mean(loss_value)))
